...
/*
 * Searches for the next configuration of bytes in the region such that hash of
 * str matches the given pattern. It starts with current configuration of bytes
 * in the region, increments the bytes until first match is found. It modifies
 * the region in str, actually the region in str is kind of state of the search
 * for all matches. The function is to be called multiple times. The bytes in
 * the region is interpreted as little endian representation of the number. Upon
 * the first call it should be 0. The search is done when the number loops back
 * to 0. Not all configuration of bytes are eligible. Only one s-th part is
 ...
 * @return 0 - if matching hash is found, 1 - if matching hash is not found and
 * all eligible configurations of bytes in the region were checked
 */
int SHA1Progress(char* hash_out, char* str, int len, int poz_start, int poz_end,
    SHA1_CTX* ctx, char* pattern, int pattern_len, int r, int s,
    int first)
{
    SHA1_CTX ctx1;
    int poz;
    ...
    while (1) {
        if (poz > poz_start) {
            if (poz == poz_end) {
                return 1; // all possible configurations of bytes in the region were
                    // checked
            }
...
void SHA1Check(char* str, int len, int poz_start, int poz_end, char* pattern,
    int pattern_len, int reminder, int denominator, FILE* f_out)
{
    SHA1_CTX ctx;
    unsigned int ii;
    ...
    int first = 1;
    while (SHA1Progress(result, str, len, poz_start, poz_end, &ctx, char_pattern,
               char_pattern_len, reminder, denominator, first)
        == 0) {
...
int main(int argc, char** argv)
{
    ...
    /* calculate hash */
    SHA1Check(fcontent, fsize, region_start, region_end, pattern, strlen(pattern),
        reminder, denominator, f_out);
